...
contextBridge.exposeInMainWorld("riskRadar", {
  ...
  watchCandidate: payload => ipcRenderer.invoke("monitor:watch-candidate", payload),
  removeMonitor: idOrName => ipcRenderer.invoke("monitor:remove", idOrName),
  runMonitorNow: idOrName => ipcRenderer.invoke("monitor:run-now", idOrName),
  ...
  onMonitorCycle: callback => subscribe("monitor:cycle", callback),
  onMonitorAlert: callback => subscribe("monitor:alert", callback),
  onMonitorError: callback => subscribe("monitor:error", callback),
